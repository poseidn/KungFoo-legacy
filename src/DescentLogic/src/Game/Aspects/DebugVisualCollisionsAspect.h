...
private:
	...
	void entityAdded(GameState & gs, Entity * ent) {
		if (ent->doesCollide() && (ent->getCollisionType() == CollisionType::BoxLike)) {
			logging::Info() << "setting up debug visual " << std::endl;

			// add debug visual
			auto tex = gs.getEngines().resourceEngine().loadImage("collision_box.png");
