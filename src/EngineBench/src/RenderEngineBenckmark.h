...
public:
	...
	void renderLightVisual(LightVisualList & lights) {
	}

	void renderSpriteVisual(SpriteVisualSVector & ent, bool ingameSprite, float deltaT);

	void renderParticleSystemVisual(ParticeSystemVisualList const& part) {
	}

	// ingameText are text visuals which are provided in game coordinates
	// rather than screen coordinates
	void renderTextVisual(TextVisualList const& part, bool ingameText) {
	}
