...
void OpenGL_ES_Render::renderLightVisual(LightVisualList & lights) {
	/*
	 glMatrixMode(GL_MODELVIEW);
	 glLoadIdentity();

	 for (LightVisual * sv : lights) {
	 if (sv->getNeedsUpdate()) {
	 const GLfloat light_ambient[] = { 1.0, 1.0, 1.0, 1.0 };

	 //GL_CHECK_ERROR(glLightfv(GL_LIGHT0, GL_POSITION, light_position));

	 GL_CHECK_ERROR(glEnable(sv->getLightNumber()));
	 GL_CHECK_ERROR(glLightfv(sv->getLightNumber(), GL_AMBIENT, light_ambient));
	 GL_CHECK_ERROR(glLightfv(sv->getLightNumber(), GL_DIFFUSE, light_ambient));

	 // important for the light to look like a spot
	 const GLfloat atten = 0.00002f;
	 GL_CHECK_ERROR(glLightf(sv->getLightNumber(), GL_QUADRATIC_ATTENUATION, atten));

	 GLfloat light_position[] = { sv->getPosition().x(), sv->getPosition().y(), sv->getPosition().z(),
	 1.0 };
	 glLightfv(sv->getLightNumber(), GL_POSITION, light_position);
	 sv->setNeedsUpdate(false);
	 }
	 }*/
}

void OpenGL_ES_Render::renderTextVisual(TextVisualList const& part, bool ingameText) {
	//return;
	...
	for (TextVisual * tv : part) {
		logging::Debug<logging::Categories::RenderEngine>() << "ensureOpenGLBind";
		...
		if (tv->getVisible()) {
			...
			if (lastBindTexture != tv->getTexture()) {
				//logging::Debug<logging::Categories::RenderEngine>() << "Binding Texture with OpenGL";
				GL_CHECK_ERROR(glBindTexture(GL_TEXTURE_2D, tv->getTexture()->getID()));
				lastBindTexture = tv->getTexture();
			}

			logging::Debug<logging::Categories::RenderEngine>() << "Setting up vertex Pointer";
			...
			logging::Debug<logging::Categories::RenderEngine>() << "Setting up tex coords";
			GL_CHECK_ERROR(glTexCoordPointer(2, GL_FLOAT, 0, &tv->getTextureBuffer().front()));

			logging::Debug<logging::Categories::RenderEngine>() << "Draw arr";
...
void OpenGL_ES_Render::renderParticleSystemVisual(ParticeSystemVisualList const& part) {
	...
	for (ParticleSystemVisual const* visual : part) {
...
void OpenGL_ES_Render::setCameraLocation(ScreenTransform const& st, Vector3 vec, bool useOffsetX,
		bool useOffsetY) {
