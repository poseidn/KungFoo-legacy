...
public:
	...
	void callFunction(std::string const& functionName, TParams ... params) {
		...
		try {
			...
			logging::Info() << " Lua exception:" << std::endl << "\t" << e.what() << std::endl << " In file: "
					<< m_fileName << std::endl << " In call: " << functionName;
			m_inactive = true;
		}
	}
...
public:
	...
	ScriptEngine(ResourceEngine & re/*, RegisterList const& regList*/);
	~ScriptEngine();
