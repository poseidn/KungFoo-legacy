...
void LevelLoader::loadFragment(std::string const& json, LevelFragment & frag) {
	...
	if (res != tinyxml2::XML_NO_ERROR) {
		logging::Error() << "cant parse xml" << std::endl;
	...
	if (int(amount) > (int(m_currentLayer) - 20)) {
		...
		for (int i = 0; i < enemyCount; i++) {
			...
			const int locY = gs.getEngines().randomEngine().randomInt(RandomEngine::SpanInt(0, frag->Size.y()))
					+ thisLayer;
			const int locX = gs.getEngines().randomEngine().randomInt(
					RandomEngine::SpanInt(0, frag->Size.x()));
			...
			const float enemyCollisionRadius = 0.7f;			// bndBox.getRadius();
			...
			if (bestPlacement.first) {
				EnemyEntity * entEnemy = fact.createMultiVisual<EnemyEntity>("enemy1", bestPlacement.second);

				gs.addEnemyEntity(entEnemy);
			} else {
