...
public:
	JavaInterface() :
			m_env(nullptr), m_libObject(nullptr), m_valid(false) {
	...
	std::string readTextFile(std::string const& fileName) {
		...
		logging::Debug<logging::Categories::Default>() << "Calling readTextFile in Java";
	...
	int playSound(std::string const& soundName, float direction) {
		logging::Debug<logging::Categories::Default>() << "Calling playSound in Java";
	...
	int playMusic(std::string const& musicName) {
		logging::Debug<logging::Categories::Default>() << "Calling playMusic in Java";
	...
	void stopPlay(int playId, float fadeOutTime) {
		logging::Debug<logging::Categories::Default>() << "Calling stopPlay in Java";
		jmethodID midPlayMusic = JavaTools::findJavaMethod(m_env, m_libObject, "stopPlay", "(IF)V");
		m_env->CallIntMethod(m_libObject, midPlayMusic, playId);
	}
