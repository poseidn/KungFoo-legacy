...
public:
	...
	SpecialMoveDetectionAspect() :
			// register all special move patterns
			m_patterns(
					{ {
					// biking kick
							InputPattern(1.5f,
									{ { new SingleNode(InputItem(InputItem::DirectionEnum::Down)),
											new SingleNode(InputItem(InputItem::DirectionEnum::Up)),
											new ConjuctionNode(InputItem(InputItem::KeyPressEnum::KeyA),
													InputItem(InputItem::KeyPressEnum::KeyB), 0.2f) } }) } }) {
