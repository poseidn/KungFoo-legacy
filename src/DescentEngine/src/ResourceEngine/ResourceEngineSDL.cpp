...
MusicPtr ResourceEngineSDL::loadMusic(std::string const& musicName) {
	...
	Mix_Music *sample;
	sample = Mix_LoadMUS((getSoundPrefix() + musicName + defaultSoundExt()).c_str());
	if (!sample) {
...
TexturePtr ResourceEngineSDL::loadImage(std::string const& imageName, unsigned int frames) {
	...
	logging::Info() << "Loading texture " << imageName;

	// todo: this surface is leaked somehow, we need to free it when the
	// texture is killed ...
	const std::string fullPath(getImagePrefix() + imageName);
...
// get the number of channels in the SDL surface
		...
		Texture * texPtr = new Texture(surface->h, surface->w, nOfColors, texture_format, surface->pixels,
				frames);
#ifdef USE_SDL
		...
		texPtr->setTextureName(imageName);
#endif

		m_textures[imageName] = texPtr;

		return TexturePtr(texPtr);
	} else {
		logging::Fatal() << "Can't load texture from file " << imageName << " from path " << fullPath;
