...
void AdvanceIntroAspect::init(IntroState & thisGs) {
	...
	{
		logging::Info() << "going to next Intro state" << std::endl;
		nextStage ( gs);
	});

	thisGs.slotActivateState.subscribe([&] ( IntroState & gs, StateChangeInfoPtr const& changeInfo )
	{
		logging::Info() << "setting up Intro state" << std::endl;
		nextStage ( gs );
	});
}
...
void AdvanceIntroAspect::nextStage(IntroState & gs) {
	...
	if (gs.getStage() == 1) {
		Entity * logoKungfu = fact.createFromTemplateName<SingleVisualEntity>("intro_logo_maker",
				Vector2(0.0f, 0.0f));
		VisualSupport::placeXCenter(logoKungfu, 0.5f, 0.60f, trans);

		logoKungfu->getActiveVisual().get().setTransparency(0.0f);
		entEngine.addEntity(logoKungfu, "logo_maker");
		gs.m_managedEntities.push_back(logoKungfu);
		...
		Entity * logoKungfu = fact.createFromTemplateName<SingleVisualEntity>("intro_logo_kungfoo",
				Vector2(0.0f, 0.0f));
		VisualSupport::placeXCenter(logoKungfu, 0.5f, 0.60f, trans);
		/*
		 ...
		 */
		entEngine.addEntity(logoKungfu, "logo_kungfoo");
		gs.m_managedEntities.push_back(logoKungfu);
		...
		// left fist
		Entity * logoFistLeft = fact.createFromTemplateName<SingleVisualEntity>("intro_logo_fist_left",
				Vector2(-10.0f, 0.0f));
		logoFistLeft->getActiveVisual().get().setIngame(trans, Vector2(100.0, 0.0f), false);

		entEngine.addEntity(logoFistLeft, "logo_fist_left");
		gs.m_managedEntities.push_back(logoFistLeft);
		m_cinEngine.addTimelineItem(
				std::make_shared < CinematicTransformAnimation
		...
		// rigth fist
		Entity * logoFistRight = fact.createFromTemplateName<SingleVisualEntity>("intro_logo_fist_right",
				Vector2(300.0f, 3.0f));
		logoFistRight->getActiveVisual().get().setIngame(trans, Vector2(), false);
		entEngine.addEntity(logoFistRight, "logo_fist_right");
		gs.m_managedEntities.push_back(logoFistRight);
		m_cinEngine.addTimelineItem(
				std::make_shared < CinematicTransformAnimation
		...
		Entity * logoBar = fact.createFromTemplateName<SingleVisualEntity>("intro_logo_bar", logoBarPos);
		logoBar->getActiveVisual().get().setIngame(trans, logoBarPos, false);
		logoBar->getActiveVisual().get().setTransparency(0.0f);
		entEngine.addEntity(logoBar, "logo_bar");
		gs.m_managedEntities.push_back(logoBar);
		/*m_cinEngine.addTimelineItem(
