...
public:
	struct LoadedVisual: boost::noncopyable {
	...
	typedef boost::ptr_list<LoadedVisual> LoadedVisualList;
	...
	void addVisual(TextureId id, SpriteVisual pVisual) {
		m_loadedVisuals.push_back(new LoadedVisual(id, pVisual));
	}
	/*
	...
	void unregister(Engines & eg) CPP11_OVERRIDE
	{
		...
		// delete all visuals, only one animation will be know to the
